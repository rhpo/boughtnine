...
func (w *World) SwitchToLevel(levelIndex int) {
	...
	println("Scheduled level switch to", levelIndex)
}
...
func (w *World) SelectLevel(index int) {
	...
	println("Switching to level", index)

	// Clear all existing objects
	w.mutex.Lock()
	println("Clearing", len(w.Objects), "objects")
	...
	println("Level", index, "loaded with", len(w.Objects), "objects")
}
